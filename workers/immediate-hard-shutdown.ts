...
zeebe.createWorker({
  taskType: "immediate-hard-shutdown",
  ...
  taskHandler: async (job) => {
    ...
    console.log(`Initiating immediate hard shutdown for turbine: ${turbineID}`);

    try {
      // Update turbine status to "emergency shut down"
      const [result] = await pool.execute(
        `UPDATE TurbinesTable SET status = ?, last_updated = NOW() WHERE turbine_id = ?`,
        ["emergency shut down", turbineID]
      );
      ...
      console.log(
        `✓ Turbine ${turbineID} status updated to "emergency shut down"`
      );
      console.log(`🚨 EMERGENCY HARD SHUTDOWN EXECUTED 🚨`);
      console.log("--------------------------");
...
console.log("✓ immediate-hard-shutdown worker initialized");
